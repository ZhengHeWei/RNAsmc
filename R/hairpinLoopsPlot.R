##plot hairpin loops
hairpinLoopsPlot <- function(ctFile){
  ...
  if(length(loops) != 0){
    ...
    ranges=data.frame(min=arr_min,max=arr_max,col=13,
                      desc="hairpin loop"
    )
    ...
    ####
    attr(loops,"1.Number of base pairs in hairpin loops") <- length_loop
    attr(loops,"2.Number of hairpin loops") <- length(loops)
    attr(loops,"3.Average length of a hairpin loop") <- length_loop/length(loops)
    attr(loops,"4.Maximum length of a hairpin loop") <- max_length
    attr(loops,"5.Minimum length of a hairpin loop") <- min_length
    attr(loops,"6.Base positions in hairpin loop") <- paste(unlist(loops),collapse = " ",sep = "")
    return(loops)
  }else{
    print("------------------------------------------------------")
    print("*                                                    *")
    ...
    print("*                                                    *")
    print("------------------------------------------------------")
    print("There is no hairpin loop")
  }

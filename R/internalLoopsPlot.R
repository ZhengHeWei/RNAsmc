##plot hairpin loops
internalLoopsPlot <- function(ctFile){
  ...
  if(length(loops) != 0){
    ...
    ranges=data.frame(min=arr_min,max=arr_max,col=7,
                      desc="internal loop"
    )
    ...
    ####
    attr(loops,"1.Number of base pairs in internal loops") <- length_loop
    attr(loops,"2.Number of internal loops") <- length(loops)
    attr(loops,"3.Average length of a internal loop") <- length_loop/length(loops)
    attr(loops,"4.Maximum length of a internal loop") <- max_length
    attr(loops,"5.Minimum length of a internal loop") <- min_length
    attr(loops,"6.Base positions in internal loop") <- paste(unlist(loops),collapse = " ",sep = "")
    return(loops)
  }else{
    print("------------------------------------------------------")
    print("*                                                    *")

##plot hairpin loops
multiBranchLoopsPlot <- function(ctFile){
  ...
  if(length(loops) != 0){
    ...
    ranges=data.frame(min=arr_min,max=arr_max,col=10,
    ...
    ####
    attr(loops,"1.Number of base pairs in multi-branch loops") <- length_loop
    attr(loops,"2.Number of multi-branch loops") <- length(loops)
    attr(loops,"3.Average length of a multi-branch loop") <- length_loop/length(loops)
    attr(loops,"4.Maximum length of a multi-branch loop") <- max_length
    attr(loops,"5.Minimum length of a multi-branch loop") <- min_length
    attr(loops,"6.Base positions in multi-branch loop") <- paste(unlist(loops),collapse = " ",sep = "")
    return(loops)
  }else{
    print("------------------------------------------------------")
    print("*                                                    *")
    ...
    print("*                                                    *")
    print("------------------------------------------------------")
    print("There is no multi-branch loop")
  }
